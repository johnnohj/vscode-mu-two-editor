#!/usr/bin/env python3
"""
Create three-color Blinka font WITHOUT base outline glyph
Test if the base outline is causing COLR layer coordinate interference
"""
...
def create_no_base_three_color_blinka():
    """Create three-color Blinka with NO base outline glyph"""

    print("Creating three-color Blinka font WITHOUT base outline glyph...")
    print("Testing if base outline interferes with COLR layer coordinates")
    ...
    scale_factor = 4
    base_offset = 50  # Back to original offset
    ...
    def create_color_layer_glyph(pixel_positions, color_name):
        """Create a layer glyph (EXACT same as working versions)"""
        pen = TTGlyphPen(None)
    ...
    def create_empty_base_outline_glyph():
        """Create EMPTY base outline - no interference with color layers"""
        pen = TTGlyphPen(None)
        # Return completely empty glyph - no outline at all
        return pen.glyph()
    ...
    # Setup glyphs - base outline is now EMPTY
    glyphs = {
        ".notdef": create_notdef_glyph(),
        "space": create_space_glyph(),
        "blinka": create_empty_base_outline_glyph(),  # EMPTY base glyph
    ...
    # Font info
    fb.setupNameTable({
        "familyName": "FreeMono No Base Three Color Blinka",
        "styleName": "Regular",
        "uniqueFontIdentifier": "FreeMonoNoBaseThreeColorBlinka-1.0",
        "fullName": "FreeMono No Base Three Color Blinka Regular",
        "psName": "FreeMonoNoBaseThreeColorBlinka-Regular",
        "version": "1.0"
    })
    ...
    try:
        ...
        print(f"\nAdded COLR/CPAL with EMPTY base outline:")
        print(f"  No base outline interference with color layers")
    ...
    # Save the font
    output_path = "FreeMono-NoBaseThreeColor-Blinka.ttf"
    font.save(output_path)

    print(f"\nNo-base three-color Blinka font created: {output_path}")
    print(f"Blinka character: \\uE000")

    if success:
        print("* Three-color COLR/CPAL with NO base outline glyph")
        print("* Should eliminate base outline interference with layer coordinates")
        print("* Test if this fixes the '3 spots too early' alignment issue")
    else:
...
if __name__ == "__main__":
    create_no_base_three_color_blinka()