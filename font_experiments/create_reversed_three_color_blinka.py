#!/usr/bin/env python3
"""
Create three-color Blinka font with REVERSED layer stacking order
Test if changing the COLR layer order fixes the alignment issue
"""
...
def create_reversed_three_color_blinka():
    """Create three-color Blinka with reversed COLR layer order"""

    print("Creating reversed three-color Blinka font from BMP...")
    print("Testing reversed COLR layer stacking order: white -> blue -> purple")
    ...
    # Load and analyze the BMP (EXACT same as working 3-color)
    ...
    # Define our three specific colors (EXACT same as working 3-color)
    white_rgb = (255, 255, 255)
    light_blue_rgb = (166, 202, 240)

    # Separate pixels by our three colors (EXACT same logic as working version)
    white_pixels = []
    light_blue_pixels = []
    purple_pixels = []
    ...
    # Scale factor (same as working versions)
    scale_factor = 4

    # Create base font (EXACT same structure)
    ...
    def create_color_layer_glyph(pixel_positions, color_name):
        """Create a layer glyph for specific pixels (EXACT same as working versions)"""
        pen = TTGlyphPen(None)
    ...
    def create_base_outline_glyph():
        """Create base outline (EXACT same as working versions)"""
        pen = TTGlyphPen(None)
    ...
    # Setup glyphs (EXACT same as working version)
    glyphs = {
    ...
    # Calculate proper metrics (EXACT same as working versions)
    all_visible = white_pixels + light_blue_pixels + purple_pixels
    if all_visible:
        min_x = min(x for x, y in all_visible)
        max_x = max(x for x, y in all_visible)
    ...
    # Font info
    fb.setupNameTable({
        "familyName": "FreeMono Reversed Three Color Blinka",
        "styleName": "Regular",
        "uniqueFontIdentifier": "FreeMonoReversedThreeColorBlinka-1.0",
        "fullName": "FreeMono Reversed Three Color Blinka Regular",
        "psName": "FreeMonoReversedThreeColorBlinka-Regular",
        "version": "1.0"
    })
    ...
    # Add COLR/CPAL tables - REVERSED layer order
    try:
        # Define our color palette (EXACT same as working version)
        ...
        # COLR data - REVERSED layer order (white bottom -> blue middle -> purple top)
        print("REVERSED stacking order:")
        print("  Original: purple -> light blue -> white (bottom to top)")
        print("  Reversed: white -> light blue -> purple (bottom to top)")

        colr_data = {
            "blinka": [
                ("blinka.white", 1),      # white layer, palette index 1 (BOTTOM)
                ("blinka.lightblue", 2),  # light blue layer, palette index 2 (MIDDLE)
                ("blinka.purple", 0)      # purple layer, palette index 0 (TOP)
            ]
        }
        ...
        print(f"\nAdded COLR/CPAL with REVERSED layer order:")
        print(f"  Palette[0]: Purple RGB(128, 32, 192) (TOP layer)")
        print(f"  Palette[1]: White RGB(255, 255, 255) (BOTTOM layer)")
        print(f"  Palette[2]: Light Blue RGB(166, 202, 240) (MIDDLE layer)")
        success = True
    ...
    # Save the font
    output_path = "FreeMono-ReversedThreeColor-Blinka.ttf"
    font.save(output_path)

    print(f"\nReversed three-color Blinka font created: {output_path}")
    print(f"Blinka character: \\uE000")

    if success:
        print("* Three-color COLR/CPAL with REVERSED layer stacking")
        print("* Test if layer order affects alignment")
        print("* White pixels now render first (bottom), purple last (top)")
    else:
...
if __name__ == "__main__":
    create_reversed_three_color_blinka()