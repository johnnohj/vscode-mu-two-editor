...
/**
 * Device Manager - handles device connection state/configuration and prepares debug adapter
 * 
 * Responsible for:
 * - Managing device connection state and configuration  
 * - Preparing debug adapter with proper connection parameters
 * - Ensuring the "phone lines" can carry the signal
...
export class DeviceManager implements vscode.DebugAdapterDescriptorFactory {
	...
	createDebugAdapterDescriptor(
		...
		const adapter = new CircuitPythonDebugAdapter();
	...
	public async attachToDevice(devicePath: string, baudRate: number = 115200): Promise<boolean> {
		try {
			...
			const device: IDevice = { path: devicePath };
	...
	public async uploadAndRun(filePath: string): Promise<boolean> {
		...
		try {
			const result = await vscode.debug.customRequest(this._activeSession, 'uploadAndRun', {
				filePath: filePath
			});
	...
	public async sendToRepl(command: string): Promise<void> {
		if (this._activeSession) {
			await vscode.debug.customRequest(this._activeSession, 'evaluate', {
				expression: command,
				context: 'repl'
			});
		} else {
	...
	public async restartDevice(): Promise<void> {
		if (this._activeSession) {
			await vscode.debug.customRequest(this._activeSession, 'restart');
		} else {
			throw new Error('No active debug session');
		}
	}
